...
export const STANCE_PRESETS: Record<string, Partial<SkeletonState>> = {
  ...
  ready: {
    pelvis: { x: -3, y: 0, z: 0 },
    leftThigh: { x: -10, y: 0, z: -4 },
    leftShin: { x: 14, y: 0, z: 0 },
    rightThigh: { x: 8, y: 0, z: 4 },
    rightShin: { x: 12, y: 0, z: 0 },
    leftFoot: { x: -4, y: 0, z: 1 },
    rightFoot: { x: -20, y: 0, z: -1 }
  },
  guard: {
    pelvis: { x: -6, y: -12, z: -2 },
    leftThigh: { x: -18, y: 0, z: -7 },
    leftShin: { x: 24, y: 0, z: 0 },
    rightThigh: { x: 12, y: 0, z: 7 },
    rightShin: { x: 18, y: 0, z: 0 },
    leftFoot: { x: -6, y: -6, z: 2 },
    rightFoot: { x: -30, y: 6, z: -2 }
  },
  walk: {
    pelvis: { x: -3, y: 0, z: -3 },
    leftThigh: { x: -30, y: 0, z: -2 },
    leftShin: { x: 22, y: 0, z: 0 },
    rightThigh: { x: 26, y: 0, z: 2 },
    rightShin: { x: 16, y: 0, z: 0 },
    leftFoot: { x: -12, y: 0, z: 0 },
    rightFoot: { x: 8, y: 0, z: 0 }
  },
  run: {
    pelvis: { x: -14, y: 0, z: -5 },
    leftThigh: { x: 44, y: 0, z: 2 },
    leftShin: { x: 48, y: 0, z: 0 },
    rightThigh: { x: -46, y: 0, z: -2 },
    rightShin: { x: 34, y: 0, z: 0 },
    leftFoot: { x: -92, y: 0, z: 0 },
    rightFoot: { x: 12, y: 0, z: 0 }
  },
  kneel: {
    pelvis: { x: -18, y: 0, z: 0 },
    leftThigh: { x: 68, y: 0, z: -6 },
    leftShin: { x: 92, y: 0, z: 0 },
    rightThigh: { x: -62, y: 0, z: 6 },
    rightShin: { x: 82, y: 0, z: 0 },
    leftFoot: { x: 16, y: 0, z: 0 },
    rightFoot: { x: -24, y: 0, z: 0 }
  },
  dodge: {
    ...
    leftFoot: { x: -40, y: -10, z: 3 },
    rightFoot: { x: 2, y: 10, z: -3 }
  }
};
...
export const ACTION_PRESETS: Record<string, Partial<SkeletonState>> = {
  ...
  punch: {
    ...
    leftForearm: { x: 0, y: 0, z: -2 },
    rightUpperArm: { x: -28, y: 0, z: 36 },
    rightForearm: { x: -66, y: 0, z: 12 },
    leftHandTerminal: { x: 0, y: 0, z: -4 },
    rightHandTerminal: { x: 0, y: 0, z: 12 }
  },
  block: {
    torso: { x: -10, y: 5, z: 0 },
...
export const POSE_DEFINITIONS: Record<string, PoseDefinition> = {
  ...
  run: {
    name: 'run',
    category: 'stance',
    description: 'Forward-running silhouette for chase or urgent motion.',
    tags: ['movement', 'chase', 'speed'],
    skeleton: mergeSkeleton(STANCE_PRESETS.run, ACTION_PRESETS.none),
  ...
  punch: {
    name: 'punch',
    category: 'action',
    description: 'Forward punch with opposite hand guarding the body.',
    tags: ['attack', 'strike', 'impact'],
    skeleton: mergeSkeleton(STANCE_PRESETS.guard, ACTION_PRESETS.punch),
    variants: {
      left: mergeSkeleton(STANCE_PRESETS.guard, ACTION_PRESETS.punch),
      right: mergeSkeleton(STANCE_PRESETS.guard, mirrorSkeletonZ(ACTION_PRESETS.punch))
    },
    defaultModifiers: ['aggressive', 'extendPunch'],
    defaultHandGestures: { left: 'fist', right: 'fist' }
  },
  block: {
    name: 'block',
...
const modifierOverlays: Record<SemanticModifier, Partial<SkeletonState>> = {
  leanForward: { torso: { x: -10, y: 0, z: 0 }, pelvis: { x: -5, y: 0, z: 0 } },
  leanBackward: { torso: { x: 12, y: 0, z: 0 }, pelvis: { x: 7, y: 0, z: 0 } },
  twistTorso: { torso: { x: 0, y: 14, z: -3 }, head: { x: 0, y: 10, z: 0 } },
  extendPunch: { leftUpperArm: { x: -10, y: 0, z: 0 }, leftForearm: { x: -6, y: 0, z: 0 } },
  defensive: { torso: { x: 4, y: 0, z: 0 }, head: { x: 2, y: 0, z: 0 } },
  aggressive: { torso: { x: -6, y: 8, z: -2 }, head: { x: -2, y: 6, z: 0 } },
...
function cleanArmExtension(skeleton: SkeletonState, side: 'left' | 'right', poseName: string): SkeletonState {
  ...
  if (poseName === 'punch' || poseName === 'point' || poseName === 'push') {
  ...
  if (poseName === 'block' || poseName === 'guard') {
    next[upperKey] = {
      ...
      y: clamp(upper.y, -14, 14),
      z: clamp(upper.z, side === 'left' ? -48 : 18, side === 'left' ? -14 : 48)
    };
    next[forearmKey] = {
      x: clamp(forearm.x, -72, -34),
      y: clamp(forearm.y, -12, 12),
      z: clamp(forearm.z, side === 'left' ? -28 : 6, side === 'left' ? -6 : 28)
    };
  }
...
function applyPoseCleanup(skeleton: SkeletonState, poseName: string): SkeletonState {
  ...
  next.torso = clampJoint(next.torso, { x: [-32, 28], y: [-38, 38], z: [-24, 24] });
  next.head = clampJoint(next.head, { x: [-24, 24], y: [-34, 34], z: [-20, 20] });
  next.pelvis = clampJoint(next.pelvis, { x: [-28, 20], y: [-28, 28], z: [-24, 24] });
  ...
  next.leftUpperArm = clampJoint(next.leftUpperArm, { x: [-105, 42], y: [-34, 34], z: [-62, 36] });
  next.rightUpperArm = clampJoint(next.rightUpperArm, { x: [-105, 42], y: [-34, 34], z: [-36, 62] });
  next.leftForearm = clampJoint(next.leftForearm, { x: [-84, 24], y: [-24, 24], z: [-34, 24] });
  next.rightForearm = clampJoint(next.rightForearm, { x: [-84, 24], y: [-24, 24], z: [-24, 34] });
